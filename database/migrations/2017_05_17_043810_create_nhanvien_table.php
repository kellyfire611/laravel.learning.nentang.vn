...
class CreateNhanvienTable extends Migration {
    ...
    public function up() {
        Schema::create('cusc_nhanvien', function (Blueprint $table) {
            ...
            $table->dateTime('nv_ngaySinh')->nullable()->comment('Ngày sinh # Ngày sinh');
            $table->string('nv_diaChi', 250)->comment('Địa chỉ # Địa chỉ');
            $table->string('nv_dienThoai', 11)->comment('Điện thoại # Điện thoại');
            $table->timestamp('nv_taoMoi')->nullable()->comment('Thời điểm tạo # Thời điểm đầu tiên tạo nhân viên');
            $table->timestamp('nv_capNhat')->nullable()->comment('Thời điểm cập nhật # Thời điểm cập nhật nhân viên gần nhất');
