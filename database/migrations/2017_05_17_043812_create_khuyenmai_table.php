...
class CreateKhuyenmaiTable extends Migration {
    ...
    public function up() {
        Schema::create('cusc_khuyenmai', function (Blueprint $table) {
            ...
            $table->dateTime('km_ngayLap')->nullable()->comment('Thời điểm lập # Thời điểm lập kế hoạch khuyến mãi');
            ...
            $table->timestamp('km_taoMoi')->nullable()->comment('Thời điểm tạo # Thời điểm đầu tiên tạo chương trình khuyến mãi');
            $table->timestamp('km_capNhat')->nullable()->comment('Thời điểm cập nhật # Thời điểm cập nhật chương trình khuyến mãi gần nhất');
